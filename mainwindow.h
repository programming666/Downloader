...
private:
    Ui::MainWindow *ui;
    DownloadManager& m_downloadManager; ///< 下载管理器实例。
    SettingsManager& m_settingsManager; ///< 设置管理器实例。
    HistoryManager& m_historyManager;   ///< 历史管理器实例。
    SystemTray* m_systemTray;           ///< 系统托盘实例。
    QTimer* m_uiUpdateTimer;            ///< UI更新定时器
    QSet<DownloadTask*> m_tasksToUpdate;///< 待更新UI的任务集合
